...
| **Taxonomy** | カテゴリ別一覧 | `list.erb` | カテゴリごとに 1 URL |
...
Taxonomy は全投稿のカテゴリを `uniq` し、カテゴリごとに list ページを生成します。
...
- Taxonomy はカテゴリのみ（タグ等は未対応）
- `markdown:` は Post / Home / Section / Taxonomy / Page に渡されますが、Home / Section / Taxonomy では本文 Markdown を直接レンダリングせず、主に Post / Page で使用
